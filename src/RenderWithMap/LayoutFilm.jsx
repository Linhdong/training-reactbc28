...
export default class LayoutFilm extends Component {
    ...
    renderCardFilm = () => {
        const arrJSx = data.map((film, index) => {
            return <div className='col-md-3 col-lg-2 mt-2' key={index}>
                <div className="card">
                    <img src={film.hinhAnh} alt="..." height={350} />
                    <div className="card-body bg-dark text-white">
                        <h4 style={{height:75}}>{film.tenPhim}</h4>
                        <p style={{height:75}}>{film.moTa.length > 100 ? film.moTa.slice(0,100) + '...' : film.moTa}</p>
                    </div>
                </div>
            </div>
        })
    ...
    render() {
        return (
            ...
            <div className='bg-films'>
                <div className='h-100' style={{ background: 'rgba(0,0,0,.5)'}}>
                    <nav className="navbar navbar-expand-sm navbar-dark" style={{ background: 'rgba(255,0,255,.1)' }}>
